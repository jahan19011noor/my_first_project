<?php
	...
	if(isset($_POST['admin_name']) && isset($_POST['password']))
	{
		...
		if(!empty($name) && !empty($password))
		{
			...
			$query = "SELECT id FROM admin_panel WHERE name = '".mysql_real_escape_string($name)."' AND password = '".mysql_real_escape_string($pass)."'";
...
<body>
	...
	<form action="adminvalidation.php" method="POST">
		<div id="myform">
			<ul class="name">
				<li><h1>Enter Your Name:</h1></li>
				<li><input type="text" name="admin_name" id="name"/></li>
			</ul>
			<ul class="clear"></ul>
			<ul class="pass">
			...
			<ul class="clear"></ul>
			<ul class="submit">
				<li><input type="submit" name="submit" value="Enter Admin Panel" id="submit"></li>
